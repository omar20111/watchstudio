...
/* A numeral is placed by its ink, not its text box: upright, it slides along
   ...
   `ink`: glyph-centred points in mm, x right, y down. */
export function placeNumeral(h,ringR,ink,upright){const a=h*Math.PI/6,ux=Math.sin(a),uy=-Math.cos(a);
 if(!ink||!ink.length){const c=ringR*.85;return{x:c*ux,z:c*uy,rotY:upright?0:-a}}
 if(!upright){let minY=0;for(const p of ink)if(p[1]<minY)minY=p[1];
  const c=ringR+minY;return{x:c*ux,z:c*uy,rotY:-a}}
