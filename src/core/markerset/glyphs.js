...
const REF=100;
const newCanvas=(w,h)=>{if(typeof OffscreenCanvas!=='undefined')return new OffscreenCanvas(w,h);
 const c=document.createElement('canvas');c.width=w;c.height=h;return c};
let probe=null;
const probeCtx=()=>probe||(probe=newCanvas(8,8).getContext('2d'));

const fontOf=(style,px)=>`${style.weight} ${px}px ${fontStackOf(style)}`;

/* the ink box of `txt` at REF px, centred-alignment, alphabetic baseline */
function inkBox(style,txt){const c=probeCtx();c.font=fontOf(style,REF);c.textAlign='center';c.textBaseline='alphabetic';
...
export function numeralMetrics(style,h){
 ...
 return{txt,fontMm,family:fontStackOf(style),weight:style.weight,
...
export function glyphField(style,h,ppmm){
 ...
 const key=[m.txt,style.font,style.numerals,style.weight,style.sizeMm,ppmm].join('|');
 ...
 ctx.font=fontOf(style,m.fontMm*ppmm);ctx.textAlign='center';ctx.textBaseline='alphabetic';
 if('direction'in ctx)ctx.direction='ltr';
 ctx.fillStyle='#fff';
 /* sample (i, j) is the pixel centred on canvas (i + .5, j + .5) */
 ctx.fillText(m.txt,m.ax*ppmm-f.ox+.5,m.ay*ppmm-f.oy+.5);
