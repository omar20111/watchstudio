...
import {geoOf,caseOf,outlinesOf,dialLayoutOf,dialDayOf,handLengthsOf,strapReachPx,dialTextOf,dialEdgeOf,springBarMm,RALLY_HOLES_MM} from '../geometry.js';
...
export function procOpts(part,d,sub,mode){const src=srcOf(part);const arch=caseOf(d);
 return{g:geoOf(d),arch,...clone(d.parts[src]),which:sub,hand:sub,mode,
  ...
  /* a rally strap's holes, as distances from the centre in sheet px */
  rally:src==='strap'&&d.parts.strap.style==='rally'&&d.parts.strap.variant==='leather'?RALLY_HOLES_MM.map(mm=>(springBarMm(d)+mm)*PX):null,
